...
class Bill_App:
    def __init__(self, root):
        ...
        title = Label(self.root, text="Anand Super Market", bd=12, relief=RIDGE, font=("Arial Black", 20), bg="#A569BD", fg="white").pack(fill=X)
        ...
        def time_update():
            currentt = tm.strftime ("%I:%M:%S %p")
            clock.configure(text=currentt)
        ...
        clock_label1=Label(details,text="Time:",font=("Arial Black",14),bg="#A569BD",fg="white").grid(row=0,column=6,padx=8)

        clock=Label(details,font=("Calibri",14),bg="#A569BD",fg="white")
        clock.grid(row=0,column=7,padx=10)
        time_update()
...
def total(self):
    ...
    try:
                ...
                cur = conn.cursor()
                cur.execute(
                    "insert into customer ( C_name,C_contact,Bill_no,Amount) values (%s, %s, %s, %s)",
                    (self.c_name.get(),
                     self.phone.get(),
                     self.bill_no.get(),*
                     str(self.total_all_bill),
...
def intro(self):
    ...
    self.txtarea.insert(END,"\t ANAND SUPER MARKET\n\tPhone-No.739275410")
...
def billarea(self):
    ...
    self.txtarea.insert(END,f"------------------------------------\n")
    self.txtarea.insert(END,f"Thankyou for shopping with us!!\n")
def clear(self):
        rand = random.randint(1000,9999)
        self.bill_no.set(rand)
        self.txtarea.delete(13.0,END)
        self.nutella.set(0)
        self.noodles.set(0)
        self.lays.set(0)
        self.oreo.set(0)
